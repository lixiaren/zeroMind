...
class MCTS(object):
    ...
    def _add_virtual_visits(self, path):
        for node in path:
            node._n_visits += 1

    def _revert_virtual_visits(self, path):
        for node in path:
    ...
    def _leaf_terminal_value(self, leaf):
        winner = leaf["winner"]
        if winner == -1:
            return 0.0
        return 1.0 if winner == leaf["state"].get_current_player() else -1.0

    def _finish_leaf(self, leaf, action_probs=None, leaf_value=None):
        self._revert_virtual_visits(leaf["path"])
        node = leaf["node"]
        if leaf["end"]:
            leaf_value = self._leaf_terminal_value(leaf)
        else:
    ...
    def _playout_batch(self, state, batch_size):
        ...
        for _ in range(batch_size):
            ...
            self._add_virtual_visits(leaf["path"])
            leaves.append(leaf)
