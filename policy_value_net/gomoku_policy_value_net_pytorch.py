...
class Net(nn.Module):
    ...
    def __init__(
        ...
        self.p_fc = nn.Linear(32 * board_width * board_height, board_width * board_height)
...
def _infer_arch_from_state_dict(net_params):
    ...
    return channels, n_blocks, use_attention
...
class PolicyValueNet:
    ...
    def __init__(
        ...
        if model_file:
            ...
            if inferred is not None:
                channels, n_blocks, use_attention = inferred
    ...
    def policy_value_fn(self, board):
        """Input: board; Output: (action, prob) for legal actions + value."""
        ...
        if prob_sum > 1e-12:
            ...
            legal_probs = np.ones(len(legal_positions), dtype=np.float32) / len(legal_positions)
        return zip(legal_positions, legal_probs), value

    def policy_value_fn_batch(self, boards):
        """Batch version of policy_value_fn for MCTS leaf evaluation."""
        boards = list(boards)
        if not boards:
            return []
        ...
        state_batch = np.asarray(
            [
                board.current_state().reshape(4, self.board_width, self.board_height)
                for board in boards
            ],
            dtype=np.float32,
        )
        ...
        for board, act_probs, value in zip(boards, act_probs_batch, value_batch):
            ...
            if prob_sum > 1e-12:
                ...
                legal_probs = np.ones(len(legal_positions), dtype=np.float32) / len(legal_positions)
            results.append((list(zip(legal_positions, legal_probs)), float(value)))
        return results
